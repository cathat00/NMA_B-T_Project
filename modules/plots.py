...
class TaskViz(Visualization):
  ...
  def __init__(self, task, predicted, targets_by_trial, dt=0.01):
    super().__init__()
    ...
    coords_fig = self._plot_coordinates(predicted, targets, targets_by_trial)

    # Store figures in self.figures
    self.figures = [coords_fig]
  ...
  def _plot_coordinates(self, predicted, targets, targets_by_trial):
      ...
      fig.update_layout(
          ...
          yaxis_range=[predicted.min() - 0.01, predicted.max() + 0.01],
